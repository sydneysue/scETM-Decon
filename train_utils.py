...
def get_train_instance_name(args):
    ...
    for tuple_ in (
            ...
            ('dropoutProb', args.dropout_prob, 0.1)
    ):
        if len(tuple_) == 2:
...
def save_embeddings(model, adata, embeddings, args): #epoch
    save_dict = dict(
        delta=embeddings['delta'],
        alpha=model.alpha.detach().cpu().numpy(),
        gene_names=adata.var_names,
        #rho_fixed=model.rho_fixed.detach().cpu().numpy()
    )
    if model.rho is not None:
        save_dict['rho'] = model.rho.detach().cpu().numpy()
    import pickle
    with open(os.path.join(args.ckpt_dir, 'embeddings.pkl'), 'wb') as f: #f'epoch{epoch}_'
