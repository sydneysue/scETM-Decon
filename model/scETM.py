...
#parameterization
        ...
        #self.n_topics = parameterization.get('n_topics')
        self.n_topics = args.n_topics
        #self.dropout_prob = parameterization.get('dropout_prob')
        ...
        if 'gene_emb' in adata.varm:
            ...
            self.n_celltypes = adata.varm['gene_emb'].shape[1]
        ...
        self.alpha = nn.Parameter(torch.randn(self.n_topics, self.trainable_gene_emb_dim + (self.n_celltypes if self.rho_fixed is not None else 0)))
    ...
    def get_cell_emb_weights(self, adata):
        return super().get_cell_emb_weights(adata, ['theta', 'delta'])
    ...
    def forward(self, data_dict, hyper_param_dict=dict(val=True)):
        ...
        #.clamp(self.min_logsigma, self.max_logsigma)
        
        s = logsigma_q_delta.exp()
        ok = Normal.arg_constraints["scale"].check(s)
        bad_elements = s[~ok]
        print(bad_elements)
        ...
        decon = theta @ self.alpha #F.softmax(dim =-1)
        ...
        return decon, loss, fwd_dict, tracked_items
