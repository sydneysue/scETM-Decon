...
def train(model: torch.nn.Module, adata: anndata.AnnData, args, epoch=0,
    ...
    if args.n_samplers == 1 or args.batch_size >= adata.n_obs:
        sampler = CellSampler(adata, args) #batch_size
    else:
        sampler = CellSamplerPool(args.n_samplers, adata, args) #batch_size
    sampler.start()
        
    # set up initial learning rate and optimizer
    step = epoch * steps_per_epoch
    args.lr = args.lr * (np.exp(-args.lr_decay) ** step) #parameterization.get('lr') * (np.exp(-parameterization.get('lr_decay')) ** step)
    ...
    #early_stopping = EarlyStopping(patience=8, verbose=True, path=os.path.join(args.ckpt_dir, f'model-{next_ckpt_epoch}'))
    #num_epochs = parameterization.get("epochs")
    #print(num_epochs)
    while epoch < args.n_epochs: #num_epochs
        print(f'Training: Epoch {int(epoch):5d}/{num_epochs:5d}\tNext ckpt: {next_ckpt_epoch:7d}', end='\r')
    ...
    # log tracked items
        ...
        # eval
        if epoch >= next_ckpt_epoch or epoch >= args.n_epochs: #num_epochs
            ...
            if not args.no_eval:
                if args.test_set:
                    test_tracked_items = evaluate(model, test_adata, test_data_dict, args, next_ckpt_epoch) #parameterization
                #visualize(model, adata, args, next_ckpt_epoch)
                #evaluate(model, adata, parameterization, args, next_ckpt_epoch, args.save_embeddings and epoch >= num_epochs)
        ...
        if epoch >= num_epochs:
            ...
            logging.info("Visualizing training data")
            visualize(model, adata, args)
...
#def train_evaluate(parameterization):
    ...
    #train(model, adata, batch_size, parameterization, args, epoch)
    
    #return evaluate(model, test_adata, test_data_dict, args)


def evaluate(model: scETM, adata: anndata.AnnData, test_data_dict, args, epoch): #parameterization
...
def visualize(model: scETM, adata: anndata.AnnData, args,
             device=torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')):
    model.eval()
    ...
    if not args.no_draw:
        ...
        for emb_name, emb in embeddings.items():
            draw_embeddings(adata=adata, fname=f'{emb_name}_{n_cells}cells.pdf',
...
if __name__ == '__main__':
    ...
    if args.test_set:
        test_adata = anndata.read_h5ad(args.test_path) #healthy_test
    ...
    #batch_size = best_parameters.parameters.get('batchsize')
    #print(batch_size)
    ...
    train(model, adata, args, epoch) #batch_size, best_parameters.parameters

    # train or evaluate
    #if args.eval:
        #evaluate(model, adata, args, epoch, args.save_embeddings)
    #else:
        #train(model, adata, args, epoch, parameters=parameterization)

    # Find the best parameters for training
    #best_parameters, values, experiment, model = optimize(
        #parameters=[
            #{"name": "lr", "type": "range", "bounds": [1e-6, 0.2], "log_scale": True},
            #{"name": "batchsize", "type": "range", "bounds": [16, 512]},
            #{"name": "epochs", "type": "range", "bounds": [30, 5000]},
            #{"name": "hidden_size", "type": "range", "bounds": [32, 512]},
            #{"name": "dropout_prob", "type": "range", "bounds": [0.05, 0.5]},
            #{"name": "n_topics", "type": "range", "bounds": [5, 100]}
        #],
    ...
    # Train with best parameters, find arm (parameter set) that has best mean accuracy
    #data = experiment.fetch_data() 
    #df = data.df
    #best_arm_name = df.arm_name[df['mean'] == df['mean'].max()].values[0]
    #best_arm = experiment.arms_by_name[best_arm_name]

    #logging.info(f'Best Arm: {best_arm}')
    #print(best_arm)

    #best_batch = best_arm.parameters.get('batchsize')
